'use client';
// Platform-style presentational primitives, scoped to the /ai-act module.
// Near-monochrome palette, generous spacing, near-black primary action.
// Colour tokens (--acc, --ink, --line, …) live in ai-act.css under .ai-act-theme.
    ...
    <Surface className={className}>
      {(title || action) && (
        <div className="flex items-start justify-between gap-4 border-b border-[var(--line)] px-6 py-4">
          <div>
            {title && <p className="text-sm font-semibold tracking-tight text-[var(--ink)]">{title}</p>}
...
export function AccentButton({ className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      className={cn(
        'inline-flex items-center justify-center gap-2 rounded-full bg-[var(--acc)] px-4 py-2 text-sm font-medium text-[var(--acc-contrast)]',
...
export function GhostButton({ className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      className={cn(
        'inline-flex items-center justify-center gap-2 rounded-full border border-[var(--line-strong)] bg-[var(--surface)] px-4 py-2 text-sm font-medium text-[var(--ink-muted)]',
...
export function PageHeader({
  ...
  return (
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        {eyebrow && (
          <p className="mb-2 text-xs font-medium uppercase tracking-[0.14em] text-[var(--ink-faint)]">{eyebrow}</p>
        )}
...
export function Label({ className, ...props }: React.LabelHTMLAttributes<HTMLLabelElement>) {
  return <label className={cn('mb-1.5 block text-xs font-medium text-[var(--ink-muted)]', className)} {...props} />;
}
...
export function Badge({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <span
      className={cn(
        'inline-flex items-center rounded-full border border-[var(--line-strong)] bg-[var(--acc-soft)] px-2.5 py-0.5 text-xs font-medium text-[var(--ink-muted)]',
        className
      )}
    >
      {children}
...
export function EntryCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="font-medium tracking-tight text-[var(--ink)]">{title}</p>
