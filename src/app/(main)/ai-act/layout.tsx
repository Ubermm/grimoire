// Scoped "platform" shell for the EU AI Act module. The parent (main) layout
// provides the dark global NavBar + Footer; this wraps the content area in the
// shared light surface (design tokens in src/styles/module-theme.css) so the
// /ai-act subtree shares one visual language with the FDA Audit module.
import React from 'react';
import '@/styles/module-theme.css';
import { ModuleNav } from '@/components/ai-act/ModuleNav';

export const metadata = {
  title: 'EU AI Act — Grimoire One',
  description: 'Formally verified EU AI Act compliance.',
};

export default function AIActLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="module-theme module-canvas min-h-screen pt-8 text-[var(--ink)] antialiased">
      <ModuleNav />
      <div className="ai-rise mx-auto max-w-6xl px-6 py-10 md:px-8 md:py-14">{children}</div>
    </div>
  );
}
