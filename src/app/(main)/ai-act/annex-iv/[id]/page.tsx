...
export default function AnnexIVPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <Link href={`/ai-act/registry/${id}`} className="mb-6 inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-neutral-800">
        <ArrowLeft className="h-4 w-4" /> Back to dossier
      </Link>
      <PageHeader title="Annex IV technical documentation" />
      {missing ? <p className="text-sm text-neutral-500">System not found.</p>
